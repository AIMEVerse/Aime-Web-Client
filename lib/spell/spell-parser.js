    ...
    static spellify(spell2Json)  {
        ...
        foKeys.forEach(iKey => {
            if(iKey === "_attr") { Object.assign(output_spell,firstObject[iKey]) }
                ...
                output_spell._spells.push(SpellParser.spellify(lob))
            }
        })
        return output_spell
    }
    ...
    static xml2spell  (xmlNode)  {
        ...
        if(xmlNode.attributes) {
            for(let i=0;i<xmlNode.attributes.length;++i)  {
                ...
                const attr_name = (cMap.attributes[n.name]) ?cMap.attributes[n.name] : n.name //replace htmlattribute to spell attributes (id -> _id)
        ...
        if(xmlNode?.childNodes.length>0) {
            for(let i=0;i<xmlNode.childNodes.length;++i)  {
                ...
                if(!node.nodeName.startsWith("#")) {
                    outputSpell["spells"].push(SpellParser.xml2spell(node))
                }
            }   
        }
