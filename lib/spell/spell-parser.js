...
class SpellCLIParser {
    ...
    static spellify(spell2Json)  {
        ...
        let output_spell = {_type:tkeys[0],spells:[]}
        ...
        foKeys.forEach(iKey => {
            if(iKey === "_attr") { Object.assign(output_spell,firstObject[iKey]) }
                ...
                output_spell._spells.push(SpellCLIParser.spellify(lob))
            }
        })
        return output_spell
    }
    ...
    static xml2spell  (xmlNode)  {
        ...
        let outputSpell = {spells:[]}
        const root_name = xmlNode.nodeName
        outputSpell["_type"] = (conversionMap[root_name]) ?conversionMap[root_name] : root_name
        if(xmlNode.attributes) {
            for(let i=0;i<xmlNode.attributes.length;++i)  {
                outputSpell[xmlNode.attributes[i].name] =xmlNode.attributes[i].value
            }
        }
        // console.log("Node value@@@@@@@@@@@@@@@@ ",xmlNode?.firstChild.nodeValue);
        ...
        if(xmlNode?.childNodes.length>0) {
            for(let i=0;i<xmlNode.childNodes.length;++i)  {
                ...
                if(!node.nodeName.startsWith("#")) {
                    outputSpell["spells"].push(SpellCLIParser.xml2spell(node))
                }
            }   
        }
