...
import SpellPrimitives from "./s3d-objects/spell3d-primitives"
...
class Spell3dEngine {
  ...
  constructor() {
    ...
    this.om.registerObjects(SpellPrimitives.get_objects())
  ...
  async loadWorld(spell3d_world, auto_run = true) {
  ...
  create(data) {
    ...
    if (this.om.hasObjectClass(data[_SC.NODES.type])) {
      const spell_class = this.om.getObjectClass(data[_SC.NODES.type])
  ...
  async onFrame(frame_number) {
    if (this.status == Spell3dEngineStatus.Running) {
      ...
      this.world.onFrame(this.frame_number)
...
export class Spell3dModule extends SpellModule {
  ...
  async onFrame(frame_number) {
    this.engine.onFrame(frame_number)
  }
