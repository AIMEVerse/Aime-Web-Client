...
class Spell3dWorld {
    ...
    constructor(spell_world, engine) {
        ...
        SpellUtils.merge_defaults_with_data(spell_world, world_defaults)
    ...
    on_frame(frame_number) {
        ...
        if (this.status == SpellWorldStatus.Running) {
            ...
            Object.keys(this.spell3d_objects).forEach(obj_name => {
                ...
                const off = this.spell3d_objects[obj_name].on_frame
                if (off && (typeof off === 'function')) {
                    this.spell3d_objects[obj_name].on_frame(frame_number)
                }
            })
            //update object move
            this.render()
