...
export const nano_spells = {
    ...
    "follow-keypoint": (ns_cmd) => {
        ...
        if (ns_cmd.params) {
            const data = SpellData?.objects[ns_cmd.params.detector]
            if (data) {
                const kp = data[ns_cmd.params.index]?.landmarks[ns_cmd.params.keypoint]
                ns_cmd.s3d_object._disable_frame_3d_state = false
                ns_cmd.s3d_object._position.x = kp[0] / 200
                ns_cmd.s3d_object._position.y = kp[1] / 200
                ns_cmd.s3d_object._position.z = kp[2] / 100
